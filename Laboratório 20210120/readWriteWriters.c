...
int * notepad;
pthread_cond_t writerVar;
pthread_cond_t readerVar;
int writerCount = 0;
int readerCount = 0;
int writerQueue = 0;
...
//funções auxiliares. Prioridade para os escritores.
void enterReader () {
	pthread_mutex_lock(&mutex);
	while(writerCount > 0) {
		pthread_cond_wait(&readerVar, &mutex);
	}
	readerCount++;
...
void enterWriter () {
	...
	while(readerCount > 0 || writerCount > 0) {
		pthread_cond_wait(&writerVar, &mutex);
	}
	writerCount++;
	pthread_mutex_unlock(&mutex);
}
...
void exitWriter () {
	...
	if(writerQueue) {
		...
		pthread_cond_broadcast(&readerVar);
	}
	pthread_mutex_unlock(&mutex);
}
